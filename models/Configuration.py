...
defaultValues = {
	...
	# TODO: Clarify if it is correct to set default hardware. Maybe fail startup if undefined?
	"hardware": "respeaker2",
	"pattern": "google",
...
choices = {
	...
	"pattern":
	["google", "alexa", "kiboost", "projectalice", "pgas", "custom"],
	"offListener": [
			...
			with path.open("r") as configFile:
				...
				for key, value in data.items():
					if getattr(args, key) == None:
						...
						logger.debug(
							f'"{key}" was provided as command line argument. Ignoring config file.'
						)
		except IOError:
			logger.warn(f'Unable to read configuration file "{str(path)}"')
	else:
		logger.warn(f'Configuration file "{str(path)}" not found')
...
def parseArguments():
	...
	parser.add_argument(
		"--mqttServer",
		help=
		"Defines to what mqtt server SLC should connect. Overrides any config file",
		type=str,
	)
	parser.add_argument(
		"--mqttPort",
		help=
		"Defines what port to use to connect to mqtt. Overrides any config file",
		type=str,
	)
	parser.add_argument(
		"--mqttUsername",
	...
	parser.add_argument("--leds",
						...
						type=int)
	parser.add_argument("--defaultBrightness",
						help="Set a default brightness for your leds",
						type=int)
	parser.add_argument(
		"--endFrame",
		help=
		"Respeakers, or apa102 led systems need an end frame. If your device is not working try either 255 or 0",
		type=int,
	)
	...
	parser.add_argument(
		"--enableDoA",
		help=
		"Enables sound direction of arrival on hardware capable of it. Resources greedy!",
		type=bool,
	)
	parser.add_argument("--startPattern",
						help="Define a program start led pattern",
						type=str)
	parser.add_argument("--stopPattern",
						help="Define a prorgam stop led pattern",
						type=str)
	parser.add_argument("--offPattern",
						help="Define an off led pattern",
						type=str)
	parser.add_argument("--idlePattern",
						help="Define an idle led pattern",
						type=str)
	parser.add_argument("--wakeupPattern",
						help="Define a wakeup led pattern",
						type=str)
	parser.add_argument("--speakPattern",
						help="Define a speak led pattern",
						type=str)
	parser.add_argument("--thinkPattern",
						help="Define a think led pattern",
						type=str)
	parser.add_argument("--listenPattern",
						help="Define a listen led pattern",
						type=str)
	parser.add_argument("--errorPattern",
						help="Define an error led pattern",
						type=str)
	parser.add_argument("--successPattern",
						help="Define a success led pattern",
						type=str)
	parser.add_argument("--updatingPattern",
						help="Define an updating led pattern",
						type=str)
	parser.add_argument("--callPattern",
						help="Define a call led pattern",
						type=str)
	parser.add_argument("--setupModePattern",
						help="Define a setup mode led pattern",
						type=str)
	parser.add_argument("--conErrorPattern",
						help="Define a connection error led pattern",
						type=str)
	parser.add_argument("--messagePattern",
						help="Define a message led pattern",
						type=str)
	parser.add_argument("--dndPattern",
	...
	parser.add_argument("--everloopPort",
						help="[Matrix Voice] - Define the everloop port",
						type=int)
	parser.add_argument(
		"--pureGpioPinout",
		help=
		"[Pure GPIO] - Define the broadcom gpio number of your leds, in the order you want them",
		type=list,
		default=[],
	)
	parser.add_argument(
	...
	parser.add_argument(
		...
		help=
		"Define the configuration.yml file location for hermes led control",
		type=str,
	...
	parser.add_argument(
		"--debug",
		help=
		"Enable the debug mode for the console to return more informations",
		type=bool,
	)
