...
class RespeakerMicArrayV1(Interface):
	...
	def __init__(self, hardware, vid, pid):
		...
		super(RespeakerMicArrayV1, self).__init__(self._hardware['numberOfLeds'])

		self._leds = pixel_ring.find(vid=int(vid, 16), pid=int(pid, 16))

		if self._leds is None:
			raise InterfaceInitError('Respeaker Mic Array V1 not found using pid={} and vid={}'.format(pid, vid))
